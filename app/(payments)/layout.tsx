...
const NAV_SECTIONS: LeftNavSection[] = [
  {
    id: 'payments-nav',
    items: [
      { id: 'home', label: 'Home', href: '/dashboard', icon: <HomeNavIcon /> },
      { id: 'purchases', label: 'Purchases', href: '/purchases', icon: <CartNavIcon /> },
      { id: 'invoices', label: 'Invoices', href: '/invoices', icon: <FileNavIcon /> },
      { id: 'transactions', label: 'Transactions', href: '/transactions', icon: <ArrowsNavIcon /> },
    ],
  },
]
    ...
    <div style={{ minHeight: '100vh', backgroundColor: colors.surface.lightDarker }}>
      ...
      {orgDropdownOpen && (
        <>
          ...
          >
            <div style={{ padding: `${spacing.xs} ${spacing.sm}`, fontFamily: fontFamilies.body, fontSize: typography.label.sm.fontSize, fontWeight: fontWeights.medium, color: colors.text.lowEmphasis.onLight, textTransform: 'uppercase' as const, letterSpacing: '0.5px' }}>
              Switch organization
            </div>
            {organizations.map((org) => (
              <button
                ...
                style={{
                  ...
                  padding: `${spacing.xs} ${spacing.sm}`, border: 'none',
                  backgroundColor: org.id === activeOrg.id ? colors.selectedHighlight : 'transparent',
                  borderRadius: borderRadiusSemantics.card, cursor: 'pointer', textAlign: 'left',
                  fontFamily: fontFamilies.body, transition: `background-color ${transitionPresets.fast}`,
                }}
                onMouseEnter={(e) => { if (org.id !== activeOrg.id) e.currentTarget.style.backgroundColor = colors.hover.onLight }}
                onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = org.id === activeOrg.id ? colors.selectedHighlight : 'transparent' }}
              >
                <span aria-hidden="true" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '36px', height: '36px', borderRadius: borderRadius.lg, backgroundColor: colors.brand.default, color: colors.text.highEmphasis.onDark, fontFamily: fontFamilies.body, fontSize: '13px', fontWeight: 600, flexShrink: 0 }}>
                  {org.logoInitials}
                </span>
                <div style={{ display: 'flex', flexDirection: 'column', minWidth: 0 }}>
                  <span style={{ fontSize: typography.body.sm.fontSize, fontWeight: fontWeights.medium, color: colors.text.highEmphasis.onLight, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{org.name}</span>
                  <span style={{ fontSize: typography.body.xs.fontSize, color: colors.text.lowEmphasis.onLight }}>{org.type}</span>
                </div>
                {org.id === activeOrg.id && (
                  <svg width={16} height={16} viewBox="0 0 16 16" fill="none" aria-hidden="true" style={{ marginLeft: 'auto', flexShrink: 0 }}>
                    <path d="M13.5 4.5L6 12L2.5 8.5" stroke={colors.brand.default} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                  </svg>
                )}
              </button>
            ))}
...
export default function CanopyPaymentsLayout({ children }: { children: React.ReactNode }) {
